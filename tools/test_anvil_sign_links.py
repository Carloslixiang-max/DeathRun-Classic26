...
"""
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("unique_nearest_buttons=1", report)
            self.assertIn("single_candidate_links=1", report)
            self.assertIn("ambiguous_links=0", report)
            self.assertIn("unlinked_actions=0", report)
            self.assertIn("link=STRONG", report)
