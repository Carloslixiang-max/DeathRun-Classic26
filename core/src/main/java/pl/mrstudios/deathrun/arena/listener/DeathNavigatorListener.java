...
public final class DeathNavigatorListener implements Listener {
    ...
    public void onInteract(@NotNull PlayerInteractEvent event) {
        ...
        switch (action) {
            case PREVIOUS -> this.navigator.previous(event.getPlayer(), runtime);
            case NEXT -> this.navigator.next(event.getPlayer(), runtime);
            case JUMP -> this.navigator.jump(event.getPlayer(), runtime);
            case ACTIVATE -> {
                int index = this.navigator.selectedIndex(event.getPlayer(), runtime);
                if (index >= 0)
                    this.activationService.activate(event.getPlayer(), runtime, index);
            }
        }
    }
}
